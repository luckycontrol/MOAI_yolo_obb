...
class BaseTrainer:
    ...
    def save_metrics(self, metrics):
        ...
        if epochs_done <= 0:  # 혹은 1로 처리
            epochs_done = 1
        elapsed_time = time.time() - self.train_time_start
        mean_epoch_time = elapsed_time / epochs_done
        epochs_left = self.epochs - self.epoch - 1
        if epochs_left < 0:
            epochs_left = 0
        remain_seconds = mean_epoch_time * epochs_left

        rh = int(remain_seconds // 3600)
        rm = int((remain_seconds % 3600) // 60)
        rs = int(remain_seconds % 60)
        remain_str = f"{rh:02d}:{rm:02d}:{rs:02d}"
